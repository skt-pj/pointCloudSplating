...
def main() -> None:
    ...
    # Phase 1 scanner invariants: high-resolution Camera2 JPEG is the RGB observation of record,
    # while ARCore keeps stream headroom for tracking/depth and Raw Depth is saved independently.
    require(scanner, "sessionSurfaces.add(jpegReader.getSurface())", "high-resolution JPEG surface missing")
    require(scanner, "CameraDevice.TEMPLATE_STILL_CAPTURE", "one-shot high-resolution Camera2 capture missing")
    require(scanner, "still.addTarget(jpegReader.getSurface())", "high-resolution JPEG is not a still target")
    require(scanner, "still.set(CaptureRequest.JPEG_ORIENTATION, 0)", "stable JPEG pixel orientation contract missing")
    ...
    require(capture, "frame.getAndroidCameraTimestamp()", "Camera2/ARCore timestamp correlation missing")
    require(capture, "Pose.makeInterpolated", "exposure-time pose interpolation missing")
    require(capture, '"camera2_high_resolution_jpeg"', "high-resolution RGB source marker missing")
    require(capture, '"photometric_ground_truth"', "RGB observation role missing")
