...
def main() -> None:
    ...
    if not (pipeline.find('patch-vksplat-checkpoint-android.py')
        ...
        raise SystemExit("resumable training check failed: checkpoint/legacy/hint patch order is invalid")
    ...
    for needle, why in [
        ...
        ("std::exp(std::clamp(record[52]", "legacy log-scales are not inverted"),
        ("1.0f / (1.0f + std::exp(-opacity_logit))", "legacy opacity logit is not inverted"),
